...
function prioritizeCoinOrder(coinArray) {
  ...
  const crypto = ORDERED_NAMES.CRYPTO.filter(
    coin => coinArray.indexOf(coin) !== -1
  );
  const fiat = ORDERED_NAMES.FIAT.filter(
    coin => coinArray.indexOf(coin) !== -1
  );
...
export default function computeCoinOptions(coin) {
  ...
  let marketsArray = Object.keys(markets);
  let selectedMarketsArray = marketsArray.filter(
    market => market.split("-")[0] === coin
  );
